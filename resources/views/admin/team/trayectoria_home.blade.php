@extends('layouts.admin_layout.admin_layout')

@section('miga-de-pan') 

  <span>Trayectoria</span>
@stop

@section('content')



 {{-- titulo --}}
 <div class="contenedor-admin-entidad-titulo-form-busqueda">
    <div class="admin-entidad-titulo">Trayectoria 
     <a href="{{route('get_admin_trayectoria_crear')}}">
      <span class="admin-user-boton-Crear">Crear 
       <span class="icon-add_circle_outline"></span> 
      </span>
     </a>  
    </div>
    @include('admin.trayectoria.partes.buscador')
 </div>
 <div class="admin-contiene-entidades-y-pagination">
   <div class="admin-entidad-contenedor-entidades">
     @foreach($Trayectorias as $Trayectoria)
          @include('admin.trayectoria.partes.lista')
     @endforeach
   </div>
   
 </div>

 


  

  
@stop